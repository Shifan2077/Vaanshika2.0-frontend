...
const CustomNode = React.memo(({ nodeDatum, onNodeClick, onAddChild, onEditNode, onDeleteNode, toggleNode }) => {
  ...
  const getFullName = () => {
    const firstName = nodeDatum.attributes?.firstName || '';
    const lastName = nodeDatum.attributes?.lastName || '';
  ...
  return (
    <g>
      ...
      >
        <div 
          className={`tree-node-glass ${getNodeColor()} hover:shadow-glass-strong transition-all duration-300 ${isCollapsed && hasChildren ? 'border-dashed' : ''}`}
        ...
        >
          ...
          <div className="text-center">
            <div className="font-semibold text-sm truncate">{getFullName()}</div>
